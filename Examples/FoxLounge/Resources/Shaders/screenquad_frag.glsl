...
vec4 fxaa()
{
  ...
  float dirReduce = max(
      (lumaNW + lumaNE + lumaSW + lumaSE) * (0.25 * sceneData.fxaaReduceMul),
      sceneData.fxaaReduceMin);
  
  float rcpDirMin = 1.0/(min(abs(dir.x), abs(dir.y)) + dirReduce);
  dir = min(vec2(sceneData.fxaaSpanMax, sceneData.fxaaSpanMax),
            max(vec2(-sceneData.fxaaSpanMax, -sceneData.fxaaSpanMax),
