...
void main()
{
    ...
    if(Constants.isHorizontal == 1)
    {
        for(int i = 1; i < 5; ++i)
        {
            result += texture(sampler2D(inputTexture, samplers[0]), uv + vec2(tex_offset.x * i, 0.0)).rgb * weight[i];
            result += texture(sampler2D(inputTexture, samplers[0]), uv - vec2(tex_offset.x * i, 0.0)).rgb * weight[i];
        }
    }
    else
    {
        for(int i = 1; i < 5; ++i)
        {
            result += texture(sampler2D(inputTexture, samplers[0]), uv + vec2(0.0, tex_offset.y * i)).rgb * weight[i];
            result += texture(sampler2D(inputTexture, samplers[0]), uv - vec2(0.0, tex_offset.y * i)).rgb * weight[i];
        }
    }
